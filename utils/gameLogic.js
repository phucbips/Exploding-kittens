...
// 🛡️ Sentinel: Cryptographically secure alternative to Math.random()
const secureRandom = () => {
  ...
  Object.keys(CARD_TYPES).forEach(key => {
    ...
    if (key !== 'EXPLODE' && key !== 'DEFUSE') {
      for (let i = 0; i < cardType.count; i++) {
        deck.push({
          // 🛡️ Sentinel: Secure ID generation
          id: `${key}_${i}_${globalThis.crypto.randomUUID().split('-')[0]}`,
          type: key,
          ...cardType,
...
export const initializeGame = (playerNames) => {
  ...
  const players = playerNames.map((name, index) => ({
    // 🛡️ Sentinel: Secure ID generation
    id: `player_${index}_${globalThis.crypto.randomUUID().split('-')[0]}`,
    name,
    hand: [],
    isAlive: true,
  ...
  players.forEach(player => {
    player.hand.push({
      // 🛡️ Sentinel: Secure ID generation
      id: `DEFUSE_${globalThis.crypto.randomUUID().split('-')[0]}`,
      type: 'DEFUSE',
      ...defuseType,
