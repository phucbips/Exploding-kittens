...
// Secure random float between 0 (inclusive) and 1 (exclusive)
...
export const shuffle = (array) => {
  ...
  while (currentIndex !== 0) {
    randomIndex = Math.floor(getSecureRandomFloat() * currentIndex);
    currentIndex--;
...
const getRandomVariant = (cardType) => {
    if (cardType.variants && cardType.variants.length > 0) {
        return cardType.variants[Math.floor(getSecureRandomFloat() * cardType.variants.length)];
    }
