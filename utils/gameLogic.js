...
import { generateSecureId, getSecureRandomInt } from './secureRandom';
  ...
  Object.keys(CARD_TYPES).forEach(key => {
    ...
    if (key !== 'EXPLODE' && key !== 'DEFUSE') {
      for (let i = 0; i < cardType.count; i++) {
        deck.push({
          id: `${key}_${i}_${generateSecureId(9)}`,
          type: key,
          ...cardType,
...
export const initializeGame = (playerNames) => {
  ...
  const players = playerNames.map((name, index) => ({
    id: `player_${index}_${generateSecureId(9)}`,
    name,
    hand: [],
    isAlive: true,
