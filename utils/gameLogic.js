...
import { getSecureRandomInt, getSecureRandomString } from './cryptoUtils';
  ...
  Object.keys(CARD_TYPES).forEach(key => {
    ...
    if (key !== 'EXPLODE' && key !== 'DEFUSE') {
      for (let i = 0; i < cardType.count; i++) {
        deck.push({
          id: `${key}_${i}_${getSecureRandomString()}`,
          type: key,
          ...cardType,
...
export const initializeGame = (playerNames) => {
  ...
  const players = playerNames.map((name, index) => ({
    id: `player_${index}_${getSecureRandomString()}`,
    name,
    hand: [],
    isAlive: true,
  ...
  players.forEach(player => {
    player.hand.push({
      id: `DEFUSE_${getSecureRandomString()}`,
      type: 'DEFUSE',
      ...defuseType,
